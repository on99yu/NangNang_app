...
app.post('/getBalance', async(req,res)=>{
    const data = req.body;
    var usdtBalance = await getBalance(data.tokenName,data.walletAddress);
    console.log("가격은",usdtBalance);
    res.send({"balance" : usdtBalance});
...
async function getBalance(tokenName, walletAddress){
  var contractAddress, data, balanceData;
  if(tokenName=='USDT'){
    ...
    balanceData = data.data.result/10**6;
    //console.log(data);
  }
  //console.log("가격은 ",balanceData);
  return balanceData; 
}
app.listen(port,()=>{ 
